...
function persianWeekday(jy, jm, jd) {
  ...
  // JS: 0=یکشنبه ... 6=شنبه → شمسی: 0=شنبه
    ...
    <div className={className}>
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-right hover:border-brand-400 hover:bg-brand-50/40 transition focus:outline-none focus:ring-2 focus:ring-brand-100"
        >
          ...
          >
            ...
            <div className="px-4 pt-4 pb-2">
                ...
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-1 mt-1">
                    <button
                      ...
                      className="text-[11px] px-2 py-0.5 rounded-lg bg-slate-50 text-slate-500 hover:bg-slate-100"
                    >
                      سال قبل
                    </button>
                    <button
              ...
              <div className="grid grid-cols-7 gap-1 mb-1">
                {WEEK_HEADERS.map((h) => (
                  <div key={h} className="text-center text-[11px] font-bold text-slate-400 py-1">
                    {h}
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                  const isSelected =
                    selected &&
                    selected.year === viewY &&
                    selected.month === viewM &&
                    selected.day === day;
                  const isToday =
                    today.jy === viewY && today.jm === viewM && today.jd === day;
            ...
            <div className="px-4 pb-4 flex gap-2">
              {allowClear && (
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl text-sm text-slate-500 hover:bg-slate-100"
                >
                  پاک کردن
                </button>
              )}
