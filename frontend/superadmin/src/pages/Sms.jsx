...
export default function Sms() {
  ...
  const save = async () => {
    try {
      await api('/sms', {
        method: 'PATCH',
        body: JSON.stringify({ provider, is_enabled: isEnabled, credentials }),
      });
      show('تنظیمات پیامک ذخیره شد');
    } catch (e) { show(e.message, 'error'); }
  };

  const test = async () => {
    if (!testPhone) return show('شماره را وارد کنید', 'error');
    try {
      const r = await api('/sms/test', { method: 'POST', body: JSON.stringify({ phone: testPhone }) });
      show(r.ok ? 'پیامک تست ارسال شد' : 'ارسال ناموفق: ' + (r.response || ''), r.ok ? 'success' : 'error');
    } catch (e) { show(e.message, 'error'); }
  };

  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-extrabold text-slate-800 mb-6">پیامک پلتفرم</h1>
      <div className="bg-white rounded-2xl shadow-sm p-6 space-y-5">
        <label className="flex items-center gap-2 text-sm text-slate-700">
          <input type="checkbox" checked={isEnabled} onChange={(e) => setIsEnabled(e.target.checked)} />
          پیامک پلتفرم فعال باشد
        </label>

        <div>
          <label className="block text-sm text-slate-600 mb-1">سرویس‌دهنده</label>
        ...
        <div className="flex justify-end">
          <button onClick={save} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-sm font-bold">
            <Save size={17} /> ذخیره
          </button>
        </div>
